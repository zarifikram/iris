...
class JEPA_Agent(nn.Module):
    ...
    ):  ## TO-DO: change world model to IJEPA_WorldModel
        super().__init__()
    ...
    @property
    def device(self):
        return self.actor_critic.actor_linear.weight.device
    ...
    def load(
        ...
        if load_embedder:
            self.embedder.load_state_dict(
                extract_state_dict(agent_state_dict, "embedder")
            )
        if load_world_model:
            self.world_model.load_state_dict(
                extract_state_dict(agent_state_dict, "world_model")
    ...
    def act(
        ...
        assert self.actor_critic.use_original_obs is False

        sequence_embedding = self.calculate_sequence_embedding_from_obs(obs)
        logits_actions = (
            self.actor_critic(sequence_embedding).logits_actions[:, -1] / temperature
        )
        act_token = (
            Categorical(logits=logits_actions).sample()
            if should_sample
    ...
    def calculate_sequence_embedding_from_obs(
        ...
        input_ac = rearrange(self.frame_embedder(obs), "b t l e -> b t (l e)")
        assert (
            input_ac.size(2)
            == self.frame_embedder.num_tokens * self.frame_embedder.embed_dim
        ), f"input_ac shape: {input_ac.size(2)} != {self.frame_embedder.num_tokens * self.frame_embedder.embed_dim}"

        sequence_embedding = self.sequence_embedder(input_ac)
        sequence_embedding = sequence_embedding.reshape(*b, *sequence_embedding.shape[1:])
