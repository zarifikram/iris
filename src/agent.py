...
class JEPA_Agent(nn.Module):
    def __init__(self, embedder: IJEPA_Embedder, world_model: WorldModel, actor_critic: ActorCritic): ## TO-DO: change world model to IJEPA_WorldModel
        super().__init__()
    ...
    def act(self, obs: torch.FloatTensor, should_sample: bool = True, temperature: float = 1.0) -> torch.LongTensor:
        input_ac = obs if self.actor_critic.use_original_obs else torch.clamp(self.embedder(obs, should_preprocess=True, should_postprocess=True), 0, 1)
