...
from agent import Agent, JEPA_Agent
...
class Collector:
    def __init__(self, env: Union[SingleProcessEnv, MultiProcessEnv], dataset: EpisodesDataset, episode_dir_manager: EpisodeDirManager) -> None:
        ...
        self.obs = self.env.reset()["pixels"]
        self.num_actions = self.env.action_spec.rand().shape[-1]
        self.num_envs = self.obs.shape[0]
        self.episode_ids = [None] * self.num_envs
        self.heuristic = RandomHeuristic(self.env.num_actions, self.env)

    @torch.no_grad()
    def collect(self, agent: JEPA_Agent, epoch: int, epsilon: float, should_sample: bool, temperature: float, burn_in: int, *, num_steps: Optional[int] = None, num_episodes: Optional[int] = None):
        ...
        while not should_stop(steps, episodes):
            ...
            obs = self.obs.to(agent.device)
            act = agent.act(obs, should_sample=should_sample, temperature=temperature).cpu().numpy()
            
            if random.random() < epsilon:
                act = self.heuristic.act(obs).cpu().numpy()
            act = TensorDict({"action": act}, batch_size=self.env.num_envs)
            # self.obs, reward, done, _ = self.env.step(act)
            out = self.env.step(act)
            act = out["action"]
            self.obs, reward, done, terminated, truncated = out["next"]["pixels"], out["next"]["reward"], out["next"]["done"], out["next"]["terminated"], out["next"]["truncated"]
    ...
    def add_experience_to_dataset(self, observations: List[np.ndarray], actions: List[np.ndarray], rewards: List[np.ndarray], dones: List[np.ndarray]) -> None:
        ...
        for i, (o, a, r, d) in enumerate(zip(*map(lambda arr: np.swapaxes(arr, 0, 1), [observations, actions, rewards, dones]))):  # Make everything (N, T, ...) instead of (T, N, ...)
            episode = Episode(
                observations=torch.ByteTensor(o).permute(0, 1, 2, 3).contiguous(),  # channel-first
                actions=torch.FloatTensor(a),
                rewards=torch.FloatTensor(r.squeeze(-1)),
                ends=torch.LongTensor(d.squeeze(-1)),
